        ...
        <div className="relative h-screen w-full overflow-hidden">
            {images.map((img, i) => (
                <img
                    ...
                    src={img}
                    alt={`background-${i}`}
                    className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-1000 ease-in-out ${i === index ? 'opacity-100 z-0' : 'opacity-0'
                        }`}
                />
            ))}

            <div className="absolute inset-0 bg-gradient-to-br from-indigo-900/10 via-purple-900/10 to-indigo-900/10 z-10 pointer-events-none" />
