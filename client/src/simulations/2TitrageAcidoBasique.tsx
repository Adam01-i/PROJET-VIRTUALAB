...
const FloatingToggleButtons = ({ sectionVisibility, toggleSectionVisibility }: any) => (
  <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex gap-2 z-40">
    {!sectionVisibility.config && (
      <button
        onClick={() => toggleSectionVisibility("config")}
    ...
    {!sectionVisibility.analysis && (
      <button
        onClick={() => toggleSectionVisibility("analysis")}
        className="bg-purple-600 hover:bg-purple-700 text-white p-3 rounded-full shadow-lg transition-colors"
        title="Afficher l'analyse"
      >
        <BarChart3 size={18} />
      </button>
    )}
...
export default function TitrageAcidoBasique() {
  ...
  // État de visibilité des sections
  ...
  return (
    <div className="w-full h-full relative bg-gradient-to-br from-indigo-900 via-indigo-800 to-violet-900">
      ...
      <div
        className={`absolute top-4 left-4 z-10 bg-white/95 backdrop-blur-sm rounded-lg p-4 shadow-xl w-80 border border-gray-200 ${sectionVisibility.config ? "" : "hidden"}`}
      >
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            title="Masquer/Afficher la configuration"
          >
      ...
      <div
        className={`absolute top-4 right-4 z-10 bg-white/95 backdrop-blur-sm rounded-lg p-3 w-80 border border-gray-200 shadow-xl ${sectionVisibility.observations ? "" : "hidden"}`}
      >
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 px-4 py-2 rounded-md text-white font-medium text-sm transition-colors flex items-center justify-center gap-1 ${
                titrageState.isRunning
      ...
      {/* Section d'analyse - NOUVELLE */}
      {titrageState.equivalenceAtteinte && (
        <div
          className={`absolute bottom-4 right-4 z-10 bg-white/95 backdrop-blur-sm rounded-lg p-3 w-80 border border-gray-200 shadow-xl ${sectionVisibility.analysis ? "" : "hidden"}`}
        >
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-gray-800 font-semibold flex items-center text-sm">
            ...
            <button
              onClick={() => toggleSectionVisibility("analysis")}
              className="p-1 hover:bg-gray-100 rounded"
              title="Masquer/Afficher l'analyse"
            >
              <Eye size={12} className="text-gray-500" />
            </button>
          </div>
          ...
          <div className="space-y-2">
            <div className="bg-green-50 p-2 rounded border border-green-200">
              <div className="text-xs font-semibold text-green-800 mb-1">Expérience terminée !</div>
              <div className="text-xs text-green-700">
                Précision: ±{Math.abs(titrageState.volumeEcoule - titrageState.volumeEquivalence).toFixed(2)} mL
              </div>
            </div>

            <button
              onClick={() => setShowResult(true)}
              className="w-full px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-md font-medium text-sm transition-colors flex items-center justify-center gap-2"
            >
              <Calculator size={16} />
              Rapport Détaillé
            </button>
            ...
            {experiments.length > 0 && (
              <div className="bg-blue-50 p-2 rounded border border-blue-200">
                <div className="text-xs font-semibold text-blue-800 mb-1">Historique</div>
                <div className="text-xs text-blue-700">
                  {experiments.length} expérience{experiments.length > 1 ? "s" : ""} réalisée
      ...
      {/* Boutons flottants pour les sections masquées */}
